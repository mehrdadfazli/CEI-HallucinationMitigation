...
def get_args():
    ...
    parser.add_argument('--gen_response_tag', default="response", help='generative response key in the responses json file (default: response)')
...
def main(args):
    ...
    for i in tqdm(range(len(inference_data))):
        ...
        if ground_truth[id-1]['type'] == 'generative':
            if 'response_length' in inference_data[i].keys():
                response_length_gen.append(inference_data[i]['response_length'])
            if 'length' in inference_data[i].keys():
                response_length_gen.append(inference_data[i]['length'])

            nouns = extract_nouns(inference_data[i][args.gen_response_tag])
            after_process_nouns = []
            for noun in nouns:
    ...
    if dimension['g']:
        ...
        print("Hal:\t\t", Ha_p)
        print("Cog:\t\t", Ha, "\n")
        if len(response_length_gen) > 0:
            print("Len:\t\t", np.mean(response_length_gen), "\n")
...
if __name__ == "__main__":
    ...
    #     inference_data = "AMBER_InstructBLIP_gen_response_alpha_0.5.json",
    #     evaluation_type = "g"
    # )
